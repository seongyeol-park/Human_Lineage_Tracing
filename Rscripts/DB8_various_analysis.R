...
#load dataset
tree <- read.tree('/home/users/sypark/00_Project/06_LineageTracing/db8/05_substitution/DB8_Lineage_count_table.txt.nwk')
meta_dt <- read_tsv('/home/users/sypark/00_Project/06_LineageTracing/meta_data/Lineage_tracing_summary_200209.txt') %>% filter(deadbody == 'DB8' & current_final_for_lineage == 'Y')
lng_dt <- read_tsv('/home/users/sypark/00_Project/06_LineageTracing/meta_data/Summary_per_lineage_200209.txt') %>% filter(deadbody == 'DB8')


#Draw heatmap for HM
dt <- read_tsv('/home/users/sypark/00_Project/06_LineageTracing/db8/05_substitution/DB8_47s_merged.txt.sampleN.sam5.txt.48sCall')
colnames(dt)
...
mVAF <- dt %>% select(ends_with('vafpct'),-`8_Blood_3s_merged_vafpct`) %>% apply(., 1, function(x) mean(x[x>0 & is.na(x)==F]))
dt$meanVAF <- mVAF
f_dt <- dt %>% filter(Nr1-Nr3 < 5 & Nr3 - casNo < 5 & Nr1 <45 & Nr3 > 1)
hm_mx <- f_dt %>% select(snv_id, ends_with('vafpct'), -`8_Blood_3s_merged_vafpct`) %>% as.data.frame() %>% column_to_rownames('snv_id') %>% as.matrix()
colnames(hm_mx) <- gsub('_vafpct','',colnames(hm_mx))
library(ComplexHeatmap)
library(circlize)
dim(hm_mx)
rt_annot = rowAnnotation(blood = f_dt$`8_Blood_3s_merged_vafpct`, mVAF=f_dt$meanVAF)
pdf('/home/users/sypark/00_Project/06_LineageTracing/db8/05_substitution/DB8_HM_for_tree_200107.pdf',height=14, width=10)
...
library(ComplexHeatmap)
library(circlize)
dt <- read_tsv('/home/users/sypark/00_Project/06_LineageTracing/db8/05_substitution/DB8_47s_merged.txt.sampleN.loh_id.g_fi.fi.sam5.txt.48sCall')
colnames(dt)
dt <- dt %>% mutate(snv_id = paste0(`#CHROM`,':',POS,REF,'>',ALT))
dt <- dt %>% mutate(Nr1=as.integer(str_split_fixed(`dp;Nr1;Nr2;Nr3;Nr3ids`, ';',5)[,2]), Nr3=as.integer(str_split_fixed(`dp;Nr1;Nr2;Nr3;Nr3ids`, ';',5)[,4]))
mVAF <- dt %>% select(ends_with('vafpct'),-`8_Blood_3s_merged_vafpct`) %>% apply(., 1, function(x) mean(x[x>0 & is.na(x)==F]))
...
dim(hm_mx)
rt_annot = rowAnnotation(blood = f_dt$`8_Blood_3s_merged_vafpct`, mVAF=f_dt$meanVAF)
pdf('/home/users/sypark/00_Project/06_LineageTracing/db8/05_substitution/DB8_HM_for_check_200107.pdf',height=14, width=10)
Heatmap(hm_mx, col = colorRamp2(c(0,50,100), c("blue","yellow","red")),
        right_annotation = rt_annot, row_names_gp=gpar(fontsize=9), column_names_gp=gpar(fontsize=12)
        ,cell_fun = function(j,i,x,y,width,height,fill){
          grid.rect(x=x, y=y, width=width, height=height,gp = gpar(col="gray",fill=NA))
...
file_list <- list.files('/home/users/sypark/00_Project/06_LineageTracing/db8/06_sequenza/', pattern = 'extract.RData$', recursive = T, full.names = T)
...
mean(dt$avg_n_dp)
dt %>% write_tsv('/home/users/sypark/00_Project/06_LineageTracing/db8/06_sequenza/cov_from_seqz.txt')
...
#default trees
ggtree(tree) %<+% m_meta_dt + geom_tree()+theme_tree2() +geom_tiplab(aes(label = sample_id), size=3) + coord_cartesian(xlim = c(0, 20000))
...
m_lng_dt <- lng_dt %>% filter(deadbody == 'DB8') %>% rename(taxa = lineage_id) %>% mutate(l_id = taxa) %>% select(-deadbody)
m_lng_dt <- left_join(m_lng_dt, m_meta_dt, by='taxa')
node_dt <- m_lng_dt %>% filter(n_samples >1)

#default trees

ggtree(tree) %<+% m_lng_dt + geom_tree()+theme_tree2() +geom_tiplab(aes(label = sample_id), size=3) + 
  coord_cartesian(xlim = c(0,25000))+
  scale_x_continuous(breaks=seq(0,25000,5000))
ggtree(tree) %<+% node_dt + geom_tree()+theme_tree2() +geom_label(aes(label = l_id), alpha=0.7, size=3) + coord_cartesian(xlim = c(0,24))+
...
file_list <- list.files('/home/users/sypark/00_Project/06_LineageTracing/db8/06_sequenza/clean_segments/', pattern = 'clean$', full.names = T)
for (fn in file_list){
  print(fn)
  dt <- read_tsv(fn)
  p <- list()
...
file_list <- list.files('/home/users/sypark/00_Project/06_LineageTracing/db8/05_substitution/perSample/',pattern = 'readc$', full.names = T)
...
res_tbl
res_tbl %>% write_tsv('/home/users/sypark/00_Project/06_LineageTracing/db8/05_substitution/perSample/peakVAF.txt')
length(p)
png('/home/users/sypark/00_Project/06_LineageTracing/db8/05_substitution/perSample/peakVAF_plot4.png', width=14, height=7, res=150, units="in")
plot_grid(plotlist=p[37:47], ncol=4, align="hv")
dev.off()
