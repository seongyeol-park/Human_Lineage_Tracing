...
#200826 add multiple_iterators = True at all fetch functions, output: dp;cons;vaf
...
def run_src(in_line):
	...
	for sid in id_path_dic.keys():
		...
		result_list.append(';'.join([str(nref),str(ncons),str(nvaf)]))
	
	if mttype == 'snv':
		repeat_seq='.'
	info_list=[in_line]+result_list
	return info_list
...
if __name__ == '__main__':
	...
	parser.add_argument("bam", type=str, help="BAM matching with the VCF")
	...
	while in_line:
		if in_line[0:4]=='#CHR':
			...
			for sid in id_path_dic.keys():
				out_file.write('\t'+sid+'_ref_readN;var_readN;vafpct')
			out_file.write('\n')
		elif in_line[0]=='#':
			out_file.write(in_line+'\n')
		else:   
