...
out_file=open(sys.argv[1]+'.rinp_fi','w')
...
while in_line:
	if in_line[0]=='#':
		...
		if varread <3 or chr1 not in chr_list:    #filter out
			...
			info_list=[]
			if refMedMQ - varMedMQ <10 and varMedMQ >= 40 and refMedMQ >=40 and varMedMM <5 and max(vclipv, vinsv, vdelv) < 90 and p1vaf < 0.5 and p2vaf < 0.5 and ncons < 3:
				m +=1
				out_file.write(in_line+'\n')
	in_line=in_file.readline().strip()
print(n)
