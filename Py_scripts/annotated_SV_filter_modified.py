#Arg1: input
#Arg2: OLfilter ('Y' or 'N')

#191003: add filtering out criteria of ol_vaf, vaf cutoff to exclude subclonal events
#191003:(svtype == 'DEL' or svtype == 'INV') and dist < 1000 and pnfrag > 0 and spfrag <= 1  -> removal pnfrag condition
#191112: all nbgbin -> nbgbint due to high depth of paired normal
#200414: add OLfilter option
#200515: add criterion removing SV in chr2:3314XXXX due to frequent false positives especially in 3_LSM_001B10
...
if OLfilter =='Y':
	out_file=open(sys.argv[1]+'.fi_OL','w')
elif OLfilter == 'N':
	out_file=open(sys.argv[1]+'.fi_nOL','w')
else:
	print('FATAL error:unknown OL filter argument')
	sys.exit(1)
n=0;m=0
...
#		mapq=in_indi[21]
		...
		if OLfilter == 'Y':	
			if ol_info.split(';')[5]=='NA':
				ol_vaf1=0
			else:
				ol_vaf1=float(ol_info.split(';')[5][:-1])
			if ol_info.split(';')[6]=='NA':
				ol_vaf2=0
			else:
				ol_vaf2=float(ol_info.split(';')[6][:-1])
		...
		if med_mapq1 < 40 or (max_pos1 < 2 and new1sa_max < 2) or (dist >=1500 and nbgbint1 >=5):
			bp1_problem = 'T'
		if med_mapq2 < 40 or (max_pos2 < 2 and new2sa_max < 2) or (dist >=1500 and nbgbint2 >=5):
			bp2_problem = 'T'
	
		# breakpoint failure definition and fragment number readjustment.
		bp1_failure='F'; bp2_failure = 'F'
		if med_mapq1 <5 or nbgbint1 >= 50:
			bp1_failure = 'T'
		if med_mapq2 <5 or nbgbint2 >= 50:
			bp2_failure = 'T'
		...
		if pnsa > 0 :
			...
			'blank'
		elif min(nbgbint1, nbgbint2) >= 5 and (svtype == 'TRA' or dist >= 1500):
			'blank'
		elif max(new1sa_max, new2sa_max) < 2 and max(max_pos1, max_pos2) < 2:
			'blank'
		elif bp1_problem == 'T' and bp2_problem == 'T':
			'blank'
		elif allfrag < 10 and safrag <2 and min(nbgbint1, nbgbint2) >=3 and max(nbgbint1, nbgbint2) >=10 and (svtype == 'TRA' or dist > 1500):
			'blank'
		elif allfrag < 10 and safrag == 0 and ((nbgbint1 >=3 and med_mapq2 < 30) or (nbgbint2 >=3 and med_mapq1 < 30)):
			'blank'
			...
			'blank'
		elif (svtype == 'DEL' or svtype == 'INV') and dist < 1000 and spfrag <= 1 :
			'blank'
		elif pnsc > 0 and allfrag-spfrag < 3:
			'blank'
			...
			'blank'
		elif min(vaf1, vaf2) < 20:
			'blank'
		elif (chr1=='2' and pos1 >= 33140000 and pos1 < 33150000) or (chr2 == '2' and pos2 >= 33140000 and pos2 < 33150000):
			'blank'
		elif OLfilter == 'Y':
			if max(ol_vaf1, ol_vaf2) >=5:
				'blank'
			else:
				m+=1
				out_file.write(in_line+'\n')
