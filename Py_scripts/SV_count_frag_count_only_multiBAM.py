...
#2019-09-30 modified to count fragment only
#2020-04-16 add limitation of read numbers 
#2020-05-12 modify to get multiple BAMs
...
def find_discordant_reads(chr1, pos1, ter1, chr2, pos2, ter2, pysam_file,sa_seq_list):
	...
	return([pair_true_list, sp_true_list, sa_true_list, pair_ref_list, jx_ref_list, all_ref_list, sa_seq_list])


def calc_final_count(chr1, pos1, ter1, chr2, pos2, ter2, t_bam):
	pos1=int(pos1); pos2=int(pos2); ter1=int(ter1); ter2=int(ter2)
	...
	return([t_tot_n, t_split_n, t_sa_n, t1_reftot_n, t1_refjx_n, t2_reftot_n, t2_refjx_n])
...
r_limit = 50000  # Maximum number of reads which this script reads at each position.
...
while fn_line:
	...
	t_file=pysam.AlignmentFile(fn_line,'rb') #Cancer bam
	header_list.append(f'B{n}_Ref1;B{n}_Ref2;B{n}_AllDiscordantFragments;B{n}_SplitFragments;B{n}_SATagFragments;B{n}_Vaf1;B{n}_Vaf2')
	...
	while sv_line:
		if sv_line[0]=='#':
			...
			if pos2 == '.' or svtype == 'INS': 
				this_res.append(';'.join(['.','.','.','.','.','.']))
			else:
				ter1=sv_indi[c_ter].split('to')[0]; ter2=sv_indi[c_ter].split('to')[1]
				res = calc_final_count(chr1, pos1, ter1, chr2, pos2, ter2, t_file)
				...
				this_res.append(';'.join(t_info_list))
		sv_line=sv_file.readline().strip()
	sv_file.close()
	print(len(this_res))
	line_n_list.append(len(this_res))
	res_list.append(this_res)
	fn_line = fn_file.readline().strip()


if len(list(set(line_n_list))) != 1:
	print('Fatal ERROR. output number inconssitency')
	print(line_n_list)
	sys.exit(1)
	
out_file=open(sys.argv[1]+'.SVvafm','w')
sv_file=open(sys.argv[1])
sv_line = sv_file.readline().strip()
i=0
while sv_line:
	if sv_line[0:4]=='#CHR':
		out_file.write(sv_line+'\t'+'\t'.join(header_list)+'\n')
	elif sv_line[0]=='#':
		out_file.write(sv_line+'\n')
	else:
		out_file.write(sv_line)
		for bamn in range(0,n):
			out_file.write('\t'+res_list[bamn][i])
		out_file.write('\n')
		i=i+1
	sv_line = sv_file.readline().strip()
print('done')
