		...
		if varread <3 or chr1 not in chr_list:    #filter out
			...
			for i in OLcols:
				if rasm_v == '1':
					...
					ncons=int(in_indi[i].split(';')[7])
				info_list.append(ncons)
			if (max(info_list) > 1 and refMinMQ <40) or min(info_list) > 1:
				'blank'
			elif refMedMQ - varMedMQ <10 and varMedMQ >= 40 and refMedMQ >=40 and varMedMM <5 and max(vclipv, vinsv, vdelv) < 90 and p1vaf < 0.5 and p2vaf < 0.5 and ncons < 3:
				m +=1
				out_file.write(in_line+'\n')
	in_line=in_file.readline().strip()
print(n)
